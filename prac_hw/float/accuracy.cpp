...
void print_results(const std::vector<T> &sample, const std::string &label, double true_var) {
    ...
    std::cout << "Fast variance: " << fast << ",  error = " << std::fixed << std::setprecision(1)
              << std::abs(fast - true_var) / true_var * 100 << "%\n"
              << std::defaultfloat;
    std::cout << "Two-pass:      " << two_pass << ",  error = " << std::fixed
              << std::setprecision(1) << std::abs(two_pass - true_var) / true_var * 100 << "%\n"
              << std::defaultfloat;
    std::cout << "One-pass:      " << one_pass << ",  error = " << std::fixed
              << std::setprecision(1) << std::abs(one_pass - true_var) / true_var * 100 << "%\n"
              << std::defaultfloat;
}
...
void run_experiment(size_t n, double mean, double stddev, unsigned seed = 777) {
    ...
    auto print_row = [&](std::string type, std::string method, double result) {
        ...
        std::cout << std::left << std::setw(8) << type << std::setw(12) << method << std::scientific
                  << std::setprecision(12) << result << " | Err: " << std::fixed
                  << std::setprecision(1) << rel_err * 100 << "%" << std::defaultfloat << std::endl;
    };

    std::cout << "\n-------- Test: Mean=" << mean << ", StdDev=" << stddev << ") --------"
              << std::endl;
