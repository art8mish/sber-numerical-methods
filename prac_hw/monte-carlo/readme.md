...
Пусть
```math
G \sim N(\mu,\sigma^2), \qquad L = e^G.
```
Тогда для любого $x>0$:
```math
F_L(x)=P(L\le x)=P(e^G\le x)=P(G\le \ln x)=\Phi\!\left(\frac{\ln x-\mu}{\sigma}\right),
```
где $\Phi$ - функция распределения стандартного нормального закона.

Дифференцируя $F_L(x)$ по $x$, получаем плотность логнормального распределения:
```math
f_L(x)=\frac{1}{x\sigma\sqrt{2\pi}}
\exp\!\left(-\frac{(\ln x-\mu)^2}{2\sigma^2}\right), \quad x>0.
```

Обратно, если $L$ имеет указанную плотность, то при замене $Y=\ln L$:
```math
f_Y(y)=f_L(e^y)\,e^y
=\frac{1}{\sigma\sqrt{2\pi}}
\exp\!\left(-\frac{(y-\mu)^2}{2\sigma^2}\right),
```
то есть $Y \sim N(\mu,\sigma^2)$. Следовательно экспонента нормальной случайной величины даёт логнормальную, а логарифм логнормальной случайной величины даёт нормальную.
